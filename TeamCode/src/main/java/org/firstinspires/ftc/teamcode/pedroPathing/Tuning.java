    ...
    public Tuning() {
        super("Select a Tuning OpMode", s -> {
            s.folder("Localization", l -> {
                l.add("Localization Test", guarded(LocalizationTest::new));
                l.add("Forward Tuner", guarded(ForwardTuner::new));
                l.add("Lateral Tuner", guarded(LateralTuner::new));
                l.add("Turn Tuner", guarded(TurnTuner::new));
                l.add("Offsets Tuner", guarded(OffsetsTuner::new));
            });
            s.folder("Automatic", a -> {
                a.add("Forward Velocity Tuner", guarded(ForwardVelocityTuner::new));
                a.add("Lateral Velocity Tuner", guarded(LateralVelocityTuner::new));
                a.add("Forward Zero Power Acceleration Tuner", guarded(ForwardZeroPowerAccelerationTuner::new));
                a.add("Lateral Zero Power Acceleration Tuner", guarded(LateralZeroPowerAccelerationTuner::new));
                a.add("Predictive Braking Tuner", guarded(PredictiveBrakingTuner::new));
            });
            s.folder("Manual", p -> {
                p.add("Translational Tuner", guarded(TranslationalTuner::new));
                p.add("Heading Tuner", guarded(HeadingTuner::new));
                p.add("Drive Tuner", guarded(DriveTuner::new));
                p.add("Centripetal Tuner", guarded(CentripetalTuner::new));
            });
            s.folder("Tests", p -> {
                p.add("Line", guarded(Line::new));
                p.add("Triangle", guarded(Triangle::new));
                p.add("Circle", guarded(Circle::new));
                p.add("Line90DegreeTurn", guarded(Line90DegreeTurn::new));
            });
        });
    }

    // SelectableOpMode has final lifecycle methods; wrap every selected child instead.
    private static java.util.function.Supplier<OpMode> guarded(java.util.function.Supplier<OpMode> factory) {
        return () -> new TuningSafety(factory, follower);
    }

    @Override
    public void onSelect() {
        if (follower == null) {
            follower = Constants.createGuardedFollower(hardwareMap);
...
class OffsetsTuner extends OpMode {
    ...
    private DistanceUnit offsetUnit;
    private double savedXOffset;
    private double savedYOffset;
    private boolean offsetsZeroed;
    private OffsetCalibration.Result result;

    @Override
    public void init() {
        PinpointLocalizer localizer =
                (PinpointLocalizer) follower.getPoseTracker().getLocalizer();
        pinpoint = localizer.getPinpoint();
        offsetUnit = Constants.localizerConstants.distanceUnit;
        savedXOffset = pinpoint.getXOffset(offsetUnit);
        savedYOffset = pinpoint.getYOffset(offsetUnit);
        offsetsZeroed = true;
        pinpoint.setOffsets(0, 0, offsetUnit);
    ...
    @Override
    public void init_loop() {
        telemetryM.debug("Turn the robot exactly 180 degrees in either direction, then stop.");
        telemetryM.debug("Absolute pod offsets will appear after the half-turn is complete.");
        telemetryM.update(telemetry);
    }
    ...
    public void loop() {
        ...
        double turnedDegrees = Math.toDegrees(Math.abs(totalHeading));
        telemetryM.debug("Turn completed: " + turnedDegrees + " / 180 degrees");

        if (result == null && OffsetCalibration.hasCompletedHalfTurn(totalHeading)) {
            Pose pose = follower.getPose();
            result = OffsetCalibration.calculate(pose.getX(), pose.getY(), totalHeading);
        }

        if (result == null) {
            telemetryM.debug("Calibration not ready. Keep turning; do not copy partial values.");
        } else {
            telemetryM.debug("Calibration captured. Stop the OpMode before editing Constants.");
            telemetryM.debug("Set Constants.localizerConstants strafePodX to " + result.strafePodX);
            telemetryM.debug("Set Constants.localizerConstants forwardPodY to " + result.forwardPodY);
        }
        telemetryM.update(telemetry);
    }
    ...
    public void stop() {
        if (offsetsZeroed) {
            pinpoint.setOffsets(savedXOffset, savedYOffset, offsetUnit);
            offsetsZeroed = false;
        }
    }
}
